...
;
struct __dst_alloc_list__dmemUL __dmemUL[513U];
# 1 "<stdin>"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
...
typedef unsigned char __dst_alloc_bucket_size_t;
# 73 "<stdin>"
unsigned char __dst_alloc_node_split__dmemUL[(1 << (9)) / 8];
...
struct __dst_alloc_list_base_t __dst_alloc_buckets__dmemUL[9 + 1] = {{1, 1}};
static __dst_alloc_bucket_size_t __dst_alloc_bucket_for_request__dmemUL(
        __dst_alloc_size_t request) {
    __dst_alloc_bucket_size_t bucket = 9;
    __dst_alloc_size_t size = 1;
    while (size < request) {
        size <<= 1;
        bucket -= 1;
...
static __dst_alloc_size_t __dst_alloc_index_for_node__dmemUL(
        __dst_alloc_size_t bucket_index,
        __dst_alloc_bucket_size_t bucket) {
    return 1 + ((bucket_index - (1 << bucket) + 1) << (9 - bucket));
}
static __dst_alloc_size_t __dst_alloc_node_for_index__dmemUL(
        __dst_alloc_size_t global_index,
        __dst_alloc_bucket_size_t bucket) {
    return ((global_index - 1) >> (9 - bucket)) + (1 << bucket) - 1;
}
...
__didx__Pb__i__Pe__ Strassen(__didx__Pb__i__Pe__ a,__didx__Pb__i__Pe__ b,int n,int l,int m)
{
  volatile struct __rect_packed_type_L5457R__L5458R __rect_packed_var_L5457R__L5458R[4U];
  ...
  if (__rect_packed_top_L5457R__L5458R + 1U == 4U) {;
g_fallback = true;
