...
;
struct __dst_alloc_list__dmemc __dmemc[32769U];
# 1 "<stdin>"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 73 "<stdin>"
unsigned char __dst_alloc_node_split__dmemc[(1 << (15)) / 8];
...
struct __dst_alloc_list_base_t __dst_alloc_buckets__dmemc[15 + 1] = {{1, 1}};
...
int insert_node(__didxclass_node root,__didxc str,int substring_index)
{
  ...
  if (__rect_packed_var_L5464R__L5465R[0 + __rect_packed_top_L5464R__L5465R] . local3 == '%') {
    ...
    __rect_packed_var_L5464R__L5465R[0 + __rect_packed_top_L5464R__L5465R] . local3 >= 'a' && __rect_packed_var_L5464R__L5465R[0 + __rect_packed_top_L5464R__L5465R] . local3 <= 'z'?(static_cast < void  >  (0)) : __assert_fail("ch >= 'a' && ch <= 'z'","hetero-UKP8SJ-kernel.cpp",53,__PRETTY_FUNCTION__);
...
void query_AhoCorasick(__didxclass_node root,char *query,int *substring_indexes,int *query_indexes)
{
  ...
  for (int offset = 0; ; offset++) {
    ...
    if (ch == '%') 
      break; 
    ch >= 'a' && ch <= 'z'?(static_cast < void  >  (0)) : __assert_fail("ch >= 'a' && ch <= 'z'","hetero-UKP8SJ-kernel.cpp",106,__PRETTY_FUNCTION__);
    int idx = ch - 'a';
