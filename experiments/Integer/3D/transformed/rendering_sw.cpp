...
bool guard_error_rendering_sw = false;
void guard_rendering_sw(int value,int size){
 #pragma HLS inline off 
 if(value>(1<<size)){guard_error_rendering_sw= true;}
}
...
// if so, return true
// else, return false
bool pixel_in_triangle( ap_uint<8> x, ap_uint<8> y, Triangle_2D triangle_2d )
{
...
ap_uint<8> find_min( ap_uint<8> in0, ap_uint<8> in1, ap_uint<8> in2 )
{
  if (in0 < in1)
  {
    if (in0 < in2)
...
ap_uint<8> find_max( ap_uint<8> in0, ap_uint<8> in1, ap_uint<8> in2 )
{
  if (in0 > in1)
  {
    if (in0 > in2)
...
void projection ( Triangle_3D triangle_3d, Triangle_2D *triangle_2d, ap_uint<1> angle )
{
...
bool rasterization1 ( Triangle_2D triangle_2d, ap_uint<8> max_min[], ap_uint<10> max_index[])
{
  ...
  guard_rendering_sw(max_min[1] - max_min[0],8);

  // calculate index for searching pixels
  max_index[0] = (max_min[1] - max_min[0]) * (max_min[3] - max_min[2]);
  guard_rendering_sw((max_min[1] - max_min[0]) * (max_min[3] - max_min[2]),10);

  return 0;
}

// find pixels in the triangles from the bounding box
ap_uint<9> rasterization2 ( bool flag, ap_uint<8> max_min[], ap_uint<10> max_index[], Triangle_2D triangle_2d, CandidatePixel fragment[] )
{
  // clockwise the vertices of input 2d triangle
  if ( flag )
  {
  ...
  {
    ap_uint<8> x = max_min[0] + k % max_min[4];
    guard_rendering_sw(max_min[0] + k % max_min[4],8);
    ap_uint<8> y = max_min[2] + k / max_min[4];
    guard_rendering_sw(max_min[2] + k / max_min[4],8);
...
int zculling ( ap_uint<7> counter, CandidatePixel fragments[], ap_uint<9> size, Pixel pixels[])
{

  // initilize the z-buffer in rendering first triangle for an image
  static ap_uint<8> z_buffer[MAX_X][MAX_Y];
  if (counter == 0)
  {
...
void coloringFB(ap_uint<7> counter, ap_uint<9> size_pixels, Pixel pixels[], int frame_buffer[MAX_X][MAX_Y])
{
...
void rendering_sw( Triangle_3D triangle_3ds[NUM_3D_TRI], ap_uint<8> output[MAX_X][MAX_Y])
{
  ...
  TRIANGLES: for (ap_uint<7> i = 0; i < NUM_3D_TRI; i ++ )
  {
    ...
    ap_uint<9> size_fragment = rasterization2( flag, max_min, max_index, triangle_2ds, fragment );
